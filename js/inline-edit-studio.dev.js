...
inlineEditStudio = {
	...
	init : function() {
		...
		qeRow.keyup(function(e) { if(e.which == 27) return inlineEditStudio.revert(); });
		bulkRow.keyup(function(e) { if (e.which == 27) return inlineEditStudio.revert(); });

		$('a.cancel', qeRow).click(function() { return inlineEditStudio.revert(); });
		$('a.save', qeRow).click(function() { return inlineEditStudio.save(this); });
		$('td', qeRow).keydown(function(e) { if ( e.which == 13 ) return inlineEditStudio.save(this); });

		$('a.cancel', bulkRow).click(function() { return inlineEditStudio.revert(); });
	...
	addEvents : function(r) {
		r.each(function() {
			var row = $(this);
			$('a.editinline', row).click(function() { inlineEditStudio.edit(this); return false; });
		});
	},
	...
	save : function(id) {
		...
		$.post('admin-ajax.php', params,
			function(r) {
				...
				if (r) {
					if ( -1 != r.indexOf('<tr') ) {
						$(inlineEditStudio.what+id).remove();
						$('#edit-'+id).before(r).remove();

						var row = $(inlineEditStudio.what+id);
						row.hide();

						if ( 'draft' == $('input[name="post_status"]').val() )
							row.find('td.column-comments').hide();

						inlineEditStudio.addEvents(row);
						row.fadeIn();
					} else {
						r = r.replace( /<.[^<>]*?>/g, '' );
...
$(document).ready(function(){inlineEditStudio.init();});
})(jQuery);
